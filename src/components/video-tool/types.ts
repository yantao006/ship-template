...
export type ToolField = { id: string; type: FieldType; stops?: number[] };
...
export type ToolAssetConfig = {
  ...
  actionIds?: string[]; footerHref?: string; badgeId?: string;
};
...
export type VideoToolCopy = {
  ...
  model: string; workflowLabel: string; parameters: string; expand: string; collapse: string; quantity: string; create: string;
  promo?: string;
  tabs: Record<string, { label: string; empty: string }>;
  assets: Record<string, { title: string; description: string; badge?: string; actions?: Record<string, string>; footer?: string }>;
