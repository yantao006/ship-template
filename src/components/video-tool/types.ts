...
export type ToolField = { id: string; type: FieldType; stops?: number[]; unit?: string; summary?: boolean; presentation?: 'ratio' | 'segmented'; order?: number };
export type ToolModel = {
  id: string; vendorId: string; icon: string; workflowIds: string[]; fieldIds: string[];
  options: Record<string, string[]>; stops?: Record<string, number[]>; defaults?: Record<string, FieldValue>; tags: { id: string; tone: string; icon?: string }[]; count?: number; costByDuration?: Record<number, number>;
};
...
export type VideoToolCopy = {
  ...
  fields: Record<string, string>; options: Record<string, Record<string, string>>; switchStates?: { on: string; off: string };
