...
export function ParameterField({ field, model, copy, references, value, onChange }: Props) {
  ...
  if (field.type === 'option') return <label className="vt-field" htmlFor={id}><span>{label}</span><select id={id} value={String(value)} onChange={event => onChange(event.target.value)}>{(model.options[field.id] ?? []).map(option => <option key={option} value={option}>{copy.options[field.id]?.[option]}</option>)}</select></label>;
